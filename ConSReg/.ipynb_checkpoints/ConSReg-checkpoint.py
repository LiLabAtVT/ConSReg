...
class ConSReg:
    ...
    def __init__(self):
        ...
        self.__feature_mat_list = []
        self.__feature_group = []
        self.__diff_tab_list = []
        self.__diff_name_list = []
    ...
    def preprocess(self, dap_file, diff_file, atac_file,  gff_file, upTSS = 3000, downTSS = 500, verbose = True):
        ...
        for file in diff_file:
            self.__diff_name_list.append(re.match(".*/([^/]*)$",file).group(1))
    ...
    def gen_feature_mat(self, neg_type, verbose = True):
        ...
        for diff_tab in self.__diff_tab_list:
            ...
            self.__feature_mat_list.append((feature_mat_up,feature_mat_down))
    ...
    def compute_imp_score(self, n_resampling = 200, n_jobs = 1, verbose = True):
        ...
        for (feature_mat_up, feature_mat_down) in self.__feature_mat_list:
        ...
        self.imp_scores_UR.columns = self.__diff_name_list
        self.imp_scores_DR.columns = self.__diff_name_list
    ...
    def get_feature_group(self, deep_split, verbose = True):
        ...
        for diff_tab in self.__diff_tab_list:
            ...
            self.__feature_group.append(feature_group)
    ...
    def gen_networks(self, imp_cutoff = 0.5, verbose = True):
        ...
        to_iter = zip(imp_scores_UR_list, imp_scores_DR_list, self.__feature_mat_list)
    ...
    def eval_by_cv(self, ml_engine, n_jobs = 1, verbose = True, rep = 5):
        ...
        if ml_engine != 'lglasso':
            self.__feature_group = [None] * len(self.__diff_tab_list)
        else:
            if not self.__feature_group_generated:
                print("Analysis aborted. 'lglasso' is selected but there are no feature groups generated yet")
                return(self)
        
        iter_list = zip(self.__diff_name_list, self.__diff_tab_list, self.__feature_mat_list, self.__feature_group)
